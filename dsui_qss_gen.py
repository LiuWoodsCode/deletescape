...
class ThemePalette:
    ...
    bg: QColor = field(default_factory=lambda: QColor("#1E1E1E"))
    ...
    progress_bg: QColor = field(default_factory=lambda: QColor("#333333"))
...
MONOSPACE_FAMILY = (
    ...
    "'IBM Plex Mono', 'Recursive Mono', 'Input Mono', 'Dank Mono', "
    "'Operator Mono', 'SF Mono', 'Menlo', 'Consolas', 'Monaco', "
    "'Liberation Mono', 'DejaVu Sans Mono', 'Ubuntu Mono', "
    "'Noto Sans Mono', 'Droid Sans Mono', 'Courier New', monospace"
)


def set_theme(theme: ThemePalette) -> None:
    global ACCENT, BG, PANEL, PANEL2, TEXT, MUTED, DIVIDER, PROG_BG
    ...
    PROG_BG = QColor(theme.progress_bg)
...
def generate_qt_stylesheet(
    theme: Optional[ThemePalette] = None,
    monospace_family: Optional[str] = None,
) -> str:
    """
    Generate a single Qt stylesheet string that can be applied to a QApplication
    or any container widget.

    Example:
        app.setStyleSheet(generate_qt_stylesheet())
        # or
        some_widget.setStyleSheet(generate_qt_stylesheet(custom_theme))

    Note:
        Custom-painted widgets like ToggleSwitch are still styled primarily
        by their paintEvent logic, not by QSS alone.
    """
    t = theme or get_theme()
    font_family = monospace_family or MONOSPACE_FAMILY

    accent = qcolor_css(t.accent)
    bg = qcolor_css(t.bg)
    panel = qcolor_css(t.panel)
    panel2 = qcolor_css(t.panel2)
    text = qcolor_css(t.text)
    muted = qcolor_css(t.muted)
    divider = qcolor_css(t.divider)
...
def apply_generated_stylesheet(
    target: QApplication | QWidget,
    theme: Optional[ThemePalette] = None,
    monospace_family: Optional[str] = None,
) -> str:
    """
    Generate the stylesheet, apply it, and return the stylesheet string.
    """
    stylesheet = generate_qt_stylesheet(theme=theme, monospace_family=monospace_family)
    target.setStyleSheet(stylesheet)
    return stylesheet


def make_glyph_icon(glyph: str, size: int = 18, color: Optional[QColor] = None) -> QIcon:
    icon_color = color or ACCENT
...
class Divider(QFrame):
    def __init__(self, thickness: int = 1):
        super().__init__()
        self.setObjectName("lineDivider")
...
class HeaderBar(QWidget):
    ...
    def __init__(self, title: str, show_back: bool, left_icon: Optional[QIcon] = None):
        ...
        row.addWidget(self._left_icon, 0, Qt.AlignmentFlag.AlignVCenter)
...
class NavRowItem(QFrame):
    ...
    def __init__(self, title: str, subtitle: str = "", icon: Optional[QIcon] = None):
        ...
        if icon:
            ...
            root.addWidget(icon_label, 0, Qt.AlignmentFlag.AlignVCenter)

        text_column = QVBoxLayout()
        text_column.setSpacing(2)

        self.title_lbl = QLabel(title)
        self.title_lbl.setStyleSheet(f"color: {qcolor_css(TEXT)}; font-size: 16px; font-weight: 600;")
        ...
        root.addWidget(chevron, 0, Qt.AlignmentFlag.AlignVCenter)
...
class SubHeading(QLabel):
    def __init__(self, text: str):
        super().__init__(text)
        self.setProperty("role", "subHeading")
...
class ToggleSwitch(QCheckBox):
    ...
    def paintEvent(self, event):
        ...
        track_rect = self.rect().adjusted(
            0,
            track_y,
            -(self.width() - self._track_w),
            -(self.height() - track_y - self._track_h),
        )
        ...
        thumb_rect = track_rect.adjusted(
            ...
            -(track_rect.right() - thumb_x - self._thumb + 1),
            -(track_rect.bottom() - thumb_y - self._thumb + 1),
        )
...
class InWindowDialog(QWidget):
    ...
    def __init__(
        ...
        self.setStyleSheet("background: rgba(0, 0, 0, 0.80);")
...
def styled_combo_box(items: Optional[Sequence[str]] = None, parent: Optional[QWidget] = None) -> QComboBox:
...
def apply_monospace_globally(app: QApplication) -> None:
    """
    Kept for compatibility, but now delegates to the stylesheet generator.
    """
    try:
        apply_generated_stylesheet(app)
    except Exception:
        pass


AUTO_APPLY_MONOSPACE = False


class _MonospaceEventFilter(QObject):
    def eventFilter(self, watched, event):
        if event.type() == QEvent.Type.Show:
            try:
                if AUTO_APPLY_MONOSPACE:
                    app = QApplication.instance()
                    if app is not None:
                        apply_generated_stylesheet(app)
                        disable_auto_monospace()
            except Exception:
                pass
        return super().eventFilter(watched, event)
...
def _install_event_filter_on_app(app: QApplication) -> None:
    global _mono_filter
    if _mono_filter is None:
        _mono_filter = _MonospaceEventFilter(app)
        app.installEventFilter(_mono_filter)
        QTimer.singleShot(
            0,
            lambda: apply_generated_stylesheet(app) if AUTO_APPLY_MONOSPACE else None,
        )


def enable_auto_monospace() -> None:
    app = QApplication.instance()
    if app is not None:
        _install_event_filter_on_app(app)
        return

    orig_init = getattr(QApplication, "__init__", None)
    if orig_init and not getattr(QApplication, "___monospace_patched", False):
        def new_init(self, *args, **kwargs):
            orig_init(self, *args, **kwargs)
            try:
                _install_event_filter_on_app(self)
            except Exception:
                pass

        QApplication.__init__ = new_init
        QApplication.___monospace_patched = True


def disable_auto_monospace() -> None:
    global _mono_filter
    try:
        app = QApplication.instance()
        if app is not None and _mono_filter is not None:
            app.removeEventFilter(_mono_filter)
    except Exception:
        pass
    _mono_filter = None
...
__all__ = [
    ...
    "set_theme",
    "get_theme",
    "generate_qt_stylesheet",
    "apply_generated_stylesheet",
    "ACCENT",
    "BG",
    "PANEL",
    "PANEL2",
