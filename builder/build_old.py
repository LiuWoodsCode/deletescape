...
def _fail_on_python_syntax_errors(root: Path) -> None:
    ...
    for py_file in root.rglob("*.py"):
        ...
        try:
            ...
            print(f"{rel} has no errors")
        except Exception as exc:
            print(f"{rel} errored out!!!")
            failures.append((py_file, exc))

    if failures:
        print("\nSyntax errors detected in Python files:\n")
        ...
        raise RuntimeError(
            f"Python syntax check failed for {len(failures)} file(s). Build aborted."
        )
...
def build_rootfs(source_root: Path, device_tree_path: Path, output_zip: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="deletescape_rootfs_") as temp_dir:
        ...
        _fail_on_python_syntax_errors(staged_root)
