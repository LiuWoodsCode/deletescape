...
from PySide6.QtCore import QEvent, QEasingCurve, Property, QPropertyAnimation, Qt, QSize, Signal, QTimer, QObject
...
def set_theme(theme: ThemePalette) -> None:
	global ACCENT, BG, PANEL, PANEL2, TEXT, MUTED, DIVIDER
...
# Default monospace font family to use throughout the UI
MONOSPACE_FAMILY = (
	"'JetBrains Mono', 'Fira Code', 'Cascadia Code', 'Source Code Pro', 'IBM Plex Mono', "
	"'Recursive Mono', 'Input Mono', 'Dank Mono', 'Operator Mono', 'SF Mono', 'Menlo', "
	"'Consolas', 'Monaco', 'Liberation Mono', 'DejaVu Sans Mono', 'Ubuntu Mono', 'Noto Sans Mono', "
	"'Droid Sans Mono', 'Courier New', monospace"
)


def apply_monospace_globally(app) -> None:
	"""
	Apply the monospace font family to the entire QApplication via stylesheet.
	Call this early (e.g. after creating QApplication) to make monospace the default.
	"""
	try:
		app.setStyleSheet(f"* {{ font-family: {MONOSPACE_FAMILY}; }}")
	except Exception:
		pass


# Automatically apply monospace to the app when it is created or when any widget is shown.
AUTO_APPLY_MONOSPACE = False


class _MonospaceEventFilter(QObject):
	def eventFilter(self, watched, event):
		# Apply monospace once when a widget is shown
		if event.type() == QEvent.Show:
			try:
				if AUTO_APPLY_MONOSPACE:
					app = QApplication.instance()
					if app is not None:
						apply_monospace_globally(app)
						# disable further automatic application to avoid repeated overrides
...
def _install_event_filter_on_app(app: QApplication) -> None:
	...
	if _mono_filter is None:
		...
		# schedule immediate application on next loop
		QTimer.singleShot(0, lambda: apply_monospace_globally(app) if AUTO_APPLY_MONOSPACE else None)


def enable_auto_monospace() -> None:
	"""Install automatic monospace application. Called by default."""
	# If app exists, install filter immediately
	app = QApplication.instance()
	if app is not None:
		_install_event_filter_on_app(app)
		return
	# Otherwise monkeypatch QApplication.__init__ so we can install filter when app is created
...
def disable_auto_monospace() -> None:
	"""Disable the automatic monospace installer and remove any installed filter."""
	global _mono_filter
	try:
		app = QApplication.instance()
