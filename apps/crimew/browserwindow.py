...
from flagsdialog import FlagsDialog
...
class EmbeddedBrowserWindow(QWidget):

    about_to_close = Signal()

    def __init__(self, browser, profile, forDevTools, parent=None):
        ...
        self._last_search = ""
        self._toolbar = None
        self._use_new_omnibox = flags.resolve_bool_flag("new_omnibox", default_value=False)
...
class BrowserWindow(QMainWindow):
    ...
    def __init__(self, browser, profile, forDevTools):
        ...
        if not forDevTools:
            ...
            mb.addMenu(self.create_debug_menu())
    ...
    def create_edit_menu(self):
        ...
        preferences_action.setShortcut(QKeySequence(Qt.Modifier.CTRL | Qt.Key.Key_Comma))
    ...
    def create_debug_menu(self):
        debug_menu = QMenu("Debug")
        flags_action = QAction("Flags...", self)
        flags_action.triggered.connect(self.open_flags_dialog)
        debug_menu.addAction(flags_action)
        return debug_menu
    ...
    def create_tool_bar(self):
        ...
        if self._use_new_omnibox:
            self._url_line_edit = Omnibox(flags.get_search_provider, self)
            self._url_line_edit.navigate_requested.connect(self._navigate_from_omnibox)
        else:
